...
# We should be generally correct that given 100 items of input there is a reasonable expectation that at least one item should end up on the first partition.
...
def whatp(oid,ring_size):
    ...
    * as above but for the secondary partition n+2 
    ...
    pt=[ ( (x+1,x * part),  ((  ((x +1) % ring_size) +1    ),  ((x +1) % ring_size) * part  ),  ((  ((x +2) % ring_size) +1    ),  ((x +2) % ring_size) * part  )     )     for x in range(0,ring_size)]
    #print pt
